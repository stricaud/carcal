/* filter.c — a Wireshark/tshark-compatible display-filter engine.
 *
 * Supported:
...
typedef struct node {
  ...
  char field[CFIELD_ABBREV_MAX];
  op_t op;
  char value[128];
} node_t;

struct cfilter { node_t *root; int match_all; };
...
static int field_matches(const cfield_t *f, op_t op, const char *val)
{
  switch (f->vtype) {
  case CV_UINT: {
...
static int eval(const node_t *n, cfield_t *root)
{
  if (!n) return 1;
  switch (n->type) {
  ...
  case N_EXISTS: {
    ...
    for (i = 0; i < na; i++) total += cfield_collect(root, al[i], hits, 64);
    return total > 0;
  }
  case N_CMP: {
    cfield_t *hits[64];
    ...
    for (i = 0; i < na; i++) {
      nh = cfield_collect(root, al[i], hits, 64);
      for (j = 0; j < nh; j++)
