...
#include "carcal.h"
#include "about_image.h"   /* embedded carcal photo (generated by tools/img2c.py) */

#ifndef CARCAL_PROTOS_DIR
#define CARCAL_PROTOS_DIR "protos"
#endif
#ifndef CARCAL_GRAMMARS_DIR
#define CARCAL_GRAMMARS_DIR "grammars"
#endif
...
static void persist_rules(void)
{
  ...
  if (rules_save_file(rp) != 0)
    gtcaca_dialog_message("Rules", "Could not write the rules file.\n"
      "Set CARCAL_PROTOS_DIR to a writable directory to persist rules.");
}
...
{ return gtcaca_dialog_confirm("Quit carcal", "Are you sure you want to quit?"); }
...
/* About box: render the embedded carcal photo with libcaca (via gtcaca's image
...
static void act_about(void *u)
{
  ...
  win  = gtcaca_window_new_centered(NULL, "About carcal", w, h);
  ...
  l1 = gtcaca_label_new(GTCACA_WIDGET(win), "carcal \xe2\x80\x94 a TUI packet analyzer", 2, imgh + 2);
...
static int same_flow(const carcal_l4_t *a, const carcal_l4_t *b)
{
...
static void act_follow_tcp(void *u)
{
  ...
  if (pidx < 0 || !carcal_locate_l4(&app.cap.pkts[pidx], &l4) || l4.proto != 6) {
  ...
  { long nseg = 0;
    for (i = 0; i < app.cap.count; i++) {
      carcal_l4_t a;
      cpkt_t *p = &app.cap.pkts[i];
      if (!carcal_locate_l4(p, &a) || a.proto != 6 || !same_flow(&a, &l4)) continue;
      nseg++;
...
static void act_follow_udp(void *u)
{
  ...
  if (pidx < 0 || !carcal_locate_l4(&app.cap.pkts[pidx], &l4) || l4.proto != 17) {
  ...
  for (i = 0; i < app.cap.count; i++) {
    carcal_l4_t a;
    cpkt_t *p = &app.cap.pkts[i];
    if (!carcal_locate_l4(p, &a) || a.proto != 17 || !same_flow(&a, &l4)) continue;
...
static nf_ctx_t *nf_build(void)
{
  ...
  for (k = 0; k < app.nview; k++) {
    ...
    has = carcal_locate_l4(p, &l4) && (l4.proto == 6 || l4.proto == 17);
...
static int dump_mode(const char *path, const char *expr)
{
  ...
  load_decoders();
  /* Optional port binding for scripted testing, e.g. CARCAL_BIND="udp 69 TFTP" */
  {
    const char *b = getenv("CARCAL_BIND");
  ...
  if (expr && *expr) {
    ...
    if (!flt) { fprintf(stderr, "carcal: filter: %s\n", err); capture_free(&cap); return 1; }
  }
  ...
  /* Test hook: CARCAL_SAVE=<path> writes the matched packets as pcapng. */
  {
    const char *sp = getenv("CARCAL_SAVE");
    if (sp && *sp) {
      ...
      free(keep);
      if (n < 0) fprintf(stderr, "carcal: save: %s\n", err);
...
static void apply_bind(const char *spec)   /* "udp 69 TFTP" */
{
  ...
  if (sscanf(spec, "%15s %u %63s", tr, &pt, pr) != 3) {
    fprintf(stderr, "carcal: bad -X binding '%s' (want '<udp|tcp> <port> <Proto>')\n", spec);
    return;
  }
  if (strcmp(tr, "udp") && strcmp(tr, "tcp")) {
    fprintf(stderr, "carcal: -X transport must be udp or tcp\n"); return;
  }
  add_port_rule(tr, pt, pr);
}
...
static int lua_cli(int argc, char **argv)
{
  ...
  rc = carcal_lua_run(script, &cap, flt, err, sizeof err);
  if (rc != 0) fprintf(stderr, "carcal: lua: %s\n", err);
