...
-- Shows carcal.decode_as (single dispatched decode) and carcal.decode_all
-- (every candidate way an Object<parent> group can be decoded).
--   carcal -s scripts/tftp.lua -r tftp.pcapng
...
function packet(pkt)
  ...
  local f = carcal.decode_as(pkt.payload, "TFTP")
  if f then
  ...
  for _, way in ipairs(carcal.decode_all(pkt.payload, "TFTP")) do
